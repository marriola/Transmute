...
// Module:      Sound change rule tests
// Copyright:   (c) 2026 Matt Arriola
// License:     MIT

module Transmute.Engine.Test.SoundChangeRuleTests
...
[<Fact>]
let ``Unconditional transformation in an alternation`` () =
    let input = [ "ke"; "ko" ]
    let expected = [ "ka"; "ka" ]
    let rule =
        """
...
let ``Compound sets`` () =
    ...
    let rules =
        ...
        ɣ → ∅                                    ; ɣa → a
        """
    Tests.testRules IPA rules input expected
...
let ``X-SAMPA works`` () =
    ...
    let rule =
        ...
        [-$Fricative] -> [+$Fricative]
        """
    Tests.testRules X_SAMPA rule input expected
